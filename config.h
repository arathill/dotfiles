...
static int winsize[] = { 500, 860 };

static WebKitFindOptions findopts = WEBKIT_FIND_OPTIONS_CASE_INSENSITIVE |
                                    WEBKIT_FIND_OPTIONS_WRAP_AROUND;

#define PROMPT_GO   "Przejdź do adresu:"
#define PROMPT_FIND "Znajdź na stronie:"

/* SETPROP(readprop, setprop, prompt)*/
#define SETPROP(p, q, w) { \
	.v = (char *[]){ "/bin/sh", "-c", \
		"prop=\"`(xprop -id $2 $0 | cut -d '\"' -f 2 | xargs -0 printf %b && "\
		"cat ~/.surf/bookmarks) | rofi -dmenu`\" &&" \
...
#define DOWNLOAD(d, r) { \
	.v = (char *[]){ "/bin/sh", "-c", \
		"/bin/sh -c \"(cd ~/Pobrane/ && curl -L -J -O --user-agent '$1'" \
		" --referer '$2' -b $3 -c $3 '$0');" \
		" sleep 5;\"", \
